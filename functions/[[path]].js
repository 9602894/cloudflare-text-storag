...
async function handleSearchPage(request, env) {
    ...
    if (filename) {
      try {
        ...
        if (remark && remark.trim() !== '') {
          await env.FILES.put('remark_' + safeFilename, remark.trim());
          messages.push('✅ 备注已保存：' + filename);
        } else {
          await env.FILES.delete('remark_' + safeFilename);
  ...
  if (formData.delete_file) {
    ...
    try {
      ...
      await env.FILES.delete('file_' + safeFilename);
      await env.FILES.delete('pwd_' + safeFilename);
      await env.FILES.delete('remark_' + safeFilename);
      await env.FILES.delete('meta_' + safeFilename);
  ...
  if (formData.save_file) {
    ...
    if (filename) {
      try {
        ...
        await env.FILES.put('pwd_' + safeFilename, password);
        const metadata = {
          ctime: Date.now(),
          size: content.length
        };
  ...
  for (const key of allFiles.keys) {
    if (key.name.startsWith('file_')) {
      ...
      if (searchPerformed && keyword.trim() !== '') {
        const content = await env.FILES.get(key.name);
      ...
      if (shouldInclude) {
        ...
        try {
          const metaData = await env.FILES.get(metaKey);
          if (metaData) {
            metadata = JSON.parse(metaData);
          } else {
            const fileContent = await env.FILES.get(key.name);
            metadata = {
              ctime: Date.now(),
              size: fileContent ? fileContent.length : 0
            };
            await env.FILES.put(metaKey, JSON.stringify(metadata));
          }
        } catch (e) {
  ...
  for (const key of allFiles.keys) {
    if (key.name.startsWith('remark_')) {
      ...
      try {
        const remark = await env.FILES.get(key.name);
        if (remark) {
    ...
    if (key.name.startsWith('pwd_')) {
      ...
      try {
        const password = await env.FILES.get(key.name);
        if (password) {
...
// 其他处理函数保持不变（handleReadFile, handleUploadFile, handleUpdatePassword, handleFileDownload）
// 这些函数与您原始代码中的完全相同，只是调整为ES模块格式
